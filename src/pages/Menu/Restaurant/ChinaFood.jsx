...
const ChinseFood = () => {
     ...
     return(
      <Page
       header={<Title backURL={'/restaurant'}>중식</Title>}
       footer={<Navi />}
       >
         <Search
             ...
             {open &&
              <ul>
                {content && (content.filter((result) => (result['소재지(도로명)'] || result['소재지지번']) || (result['소재지(지번)']).includes(address)).map((result,i) => (
                  ((result.업태명 == "중식" || result.업태명 == "중국식")  || (result.업태 == "중식" || result.업태 == "중국식")) && (<li key={'restaurant' + i}>
